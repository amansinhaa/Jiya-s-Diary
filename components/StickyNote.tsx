...
const StickyNote: React.FC<StickyNoteProps> = ({ text, color, rotation, title, date, sticker, fontSize, className, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`relative p-4 sm:p-6 shadow-xl transition-all hover:scale-105 hover:z-20 flex flex-col justify-between items-center text-center w-40 min-h-[160px] sm:w-56 sm:min-h-[220px] ${color} ${rotation} ${className} ${onClick ? 'cursor-pointer' : ''}`}
    >
      {/* Tape effect */}
